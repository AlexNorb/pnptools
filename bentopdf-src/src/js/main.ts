...
const init = async () => {
  ...
  if (__SIMPLE_MODE__) {
    const hideBrandingSections = () => {
      ...
      // Hide "Used by companies" section and referral badges
      const hideSections = document.querySelectorAll('.hide-section');
      ...
      if (toolsHeader) {
        ...
        if (title) {
          ...
          title.className = 'text-4xl md:text-5xl font-bold mb-3';
        }
        if (subtitle) {
    ...
    filteredCategories.forEach((category) => {
      ...
      category.tools.forEach((tool) => {
        ...
        if (tool.href) {
          ...
          toolCard.className =
            'tool-card block p-4 cursor-pointer flex flex-col items-center justify-center text-center no-underline';
        } else {
          toolCard = document.createElement('div');
          toolCard.className =
            'tool-card p-4 cursor-pointer flex flex-col items-center justify-center text-center';
