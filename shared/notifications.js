/**
 * PNP Buddy — Shared Notification System (Toast + Confirm + Prompt)
 * 
 * Ported from /layout/notifications.js for cross-tool consistency.
 * Requires Tailwind CSS v4 with the PNP theme tokens loaded.
 * 
 * Usage:
 *   Toast.show('Message', 'success');        // 'success' | 'error' | 'info'
 *   Toast.confirm('Are you sure?', () => {}); 
 *   Toast.prompt('Enter name', (val) => {});
 */
class NotificationSystem {
  ...
  prompt(message, onSubmit, title = "Input", defaultValue = "") {
    const overlay = document.createElement('div');
    overlay.className = 'fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 backdrop-blur-sm transition-opacity duration-200 opacity-0';
    ...
    dialog.innerHTML = `
      ...
      <div class="flex flex-col gap-1">
        <input type="text" id="prompt-input" maxlength="25" value="${defaultValue}" placeholder="Enter value" class="w-full h-9 px-3 text-xs rounded-lg border-2 border-theme-dark font-ui" />
      </div>
      <div class="flex gap-3 justify-end pt-2">
