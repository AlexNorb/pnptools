// --- CONSTANTS AND STATE MANAGEMENT ---
      ...
      window.addEventListener('message', (event) => {
          ...
          if (type === 'init-preview') {
              ...
              window.parent.postMessage({ type: 'preview-data-response', data: finalData }, '*');
          }
      });
      ...
      function setupScrollSync() {
        ...
        draggableRow.onscroll = () => { fixedRow.scrollLeft = draggableRow.scrollLeft; };
        fixedRow.onscroll = () => { draggableRow.scrollLeft = fixedRow.scrollLeft; };
      }
      ...
      function renderPairs(mode, pairs) {
        ...
        const draggableRowContent = pairs.map((pair, index) => {
            ...
            const frontContent = isFrontPlaceholder
              ...
              : `<img src="${pair.front}" alt="Front ${index + 1}" class="image-preview">`;
            ...
            return `
              <div class="pair-column ${isFrontPlaceholder ? "placeholder-front" : ""}">
                  ...
                  <div class="controls">
                    ...
                    <div class="copies-control">
                      <input type="number" min="1" value="${pair.copies}" 
                             ...
                             title="Number of copies" ${isFrontPlaceholder ? "disabled" : ""}>
                    </div>
                    <button class="delete-btn" onclick="deletePair(${index})" title="Delete this pair" ${isFrontPlaceholder ? "disabled" : ""}>Delete</button>
                  </div>
                  ${frontContent}
                </div>
              </div>`;
        ...
        if (state.backs.length > 0) {
          fixedRowContent = pairs.map((pair, index) => {
              let backContent;
              if (pair.isBackPlaceholder) {
                ...
                backContent = `<img src="${pair.back}" alt="Back ${index + 1}" class="image-preview">`;
              }
